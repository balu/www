...
#+begin_src C :results output :exports none :includes <stdio.h>
  char *s = "Hello";
...
Here, an ~int *~ suffices. An ~int~ in C can have at most ~8 *
sizeof(int) - 1~ prime factors (why?). This is a small amount of
...
Implemenet [[https://en.wikipedia.org/wiki/Sieve_of_Eratosthenes][Sieve of Eratosthenes]] to figure out the prime numbers and
